...
where each line has a SKU and a quantity. For example:
...
We need to allocate order lines to batches. When we’ve allocated an order line to a batch, we will send stock 
from that specific batch to the customer’s delivery address. When we allocate x units of stock to a batch, 
the available quantity is reduced by x. For example:
...
Batches have an ETA if they are currently shipping, or they may be in warehouse stock. We allocate to 
warehouse stock in preference to shipment batches. We allocate to shipment batches in order of which has the earliest ETA.
'''
...
def test_allocating_to_a_batch_less_than_available_quantity():
...
def test_allocating_same_batch_twice():
    batch = Batch("batch-001", "BLUE-VASE", qty=10, eta=date.today())
    line = OrderLine("order-ref", "BLUE-VASE", 2)

    batch.allocate(line)
    batch.allocate(line)

    assert batch.availble_quantity == 8